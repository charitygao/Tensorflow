# -*- coding: utf-8 -*-
"""
Created on Thu May 23 15:15:56 2019
...
with tf.compat.v1.Session() as sess:#sess.run() 网络节点的运算
    ...
    for epoch in range(training_epochs):
        for (x,y) in zip(train_X,train_Y):
            #sess.run(optimizer,feed_dict={train_X:x,train_Y:y}) #feed机制将真实的数据灌输到占位符上
            sess.run(optimizer)
            
        #显示训练的详细信息
        if epoch % display_step == 0:
            #loss = sess.run(cost,feed_dict ={train_X:train_X,inputdict['y']:train_Y})
            loss = sess.run(cost)
